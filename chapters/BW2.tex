...
\paragraph{Reliability}\index{reliability} A measure of the success with which the system conforms to some autoritative specification of its behaviour.

\paragraph{Failure vs Fault vs Error} A few definitions:
\begin{itemize}
  \item \textit{Failure:}\index{failure} A mechanical or algorithmic cause which, given the right conditions, produce an unexpected or incorrect result.
  \item \textit{Fault:}\index{fault} An internal unexpected problem in a system, caused by activation of a fault.
  \item \textit{Error:}\index{error} System deviation from its specification.
\end{itemize}
...
\paragraph{Error detection} Two classes of error detection techniques can be identified.
\begin{itemize}[nolistsep,noitemsep]
